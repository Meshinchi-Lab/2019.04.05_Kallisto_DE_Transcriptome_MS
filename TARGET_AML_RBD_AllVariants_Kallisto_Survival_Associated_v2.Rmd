...
```{r}
RBD <- readRDS(file.path("data/TARGET_AML_RBD_Kallisto_Quant_GeneLevel_scaledTPM_counts.RDS"))
...
```{r}
old_lncScore <- readRDS(file.path(PROJHOME, "Survival/TARGET_AML_RBD_lncRNAs_UV_upreg_pvalue_LT0.1_12.11.20.RDS"))
...
cohorts <- purrr::map(sets,function(set){
  ...
  CPM.lnc <- cpm(RBD.sub,
                 ...
                 prior.count = 0.5)
  if(save){
      write.csv(CPM.lnc,file=paste0("data/TARGET_AML_RBD_Kallisto_Quant_GeneLevel_",set,"_TMMCPM.csv"))
  }
