...
cox_glmnet_workflow <- function(CDE.train,targets,
  ...
  #CDE.validation has  patients as rows, surivival data and all genes expression values as columns.
      ...
      nonZero.train <- sparsemat_to_df(coef.trainingMod) %>% 
        ...
        left_join(., select(sig.mres, Variable, Hazard.Ratio, P.Value),
                  by=c("gene"="Variable"))
...
```{r}
cols_for_supp <- read.csv(file.path(substr(CDE,1,nchar(CDE)-3), "metadata/TARGET_AML_Clinical_Columns_for_Analysis_7.24.20.csv"))
...
```{r eval=FALSE}
polyA <- readRDS(file.path(HOME,"0000.00.03_Expression_Matrices/TARGET_AML_polyA_Kallisto_Quant_GeneLevel_scaledTPM_counts.RDS"))
rownames(polyA) <- gsub("\\.[0-9]{1,}$", "", rownames(polyA))
polyA <- polyA[,setdiff(colnames(polyA), colnames(RBD))] #no duplicates
polyA <- polyA[,intersect(colnames(polyA), merged$USI)] #requires EFS/OS data
...
# LSC17
# write.csv(LSC17,"~/RNA_seq_Analysis/2017.02.28_LSC17_Score/LSC17_Genes_Coeff_update.csv")
```
...
# Combine CDE 0531 and 1031

```{r warning=FALSE}
CDE.survdat <- read.csv("Survival/00_Archive/TARGET_AML_RBD_lncRNAs_UV_upreg_pvalue_LT0.1_CDE_8.02.19.csv") %>%
  select(USI,matches("^ENSG|LSC17.Score|LSC\\.|lnc\\.|Set|log10.WBC")) %>%
  left_join(.,select(merged,USI,
                     ...
                     everything()),
            by="USI")  %>% 
  #Fix the final.risk.group 
  mutate_at(vars(Final.Risk.Group),~case_when(
    Cyto.Fusion.Molecular.Risk_update=="Standard" & MRD.at.end.of.course.1=="Unknown" ~ "Unknown",
    TRUE ~ .
  )) %>% 
  #Jason wants this patient to remain High due to not beign in CR1 despite it not fitting the definition we laid out
  mutate_at(vars(Final.Risk.Group),~case_when(
    USI=="PAWAIG" ~ "High",
    TRUE ~ .  )) %>% 
  
    #Create an integrated risk group with lncRNA and cyto/mol
    mutate(lncRNA_plus_CM_Risk=factor(case_when(
              lnc.Pos.Neg == "Neg" & Cyto.Fusion.Molecular.Risk_update == "High" ~ "Standard",
              lnc.Pos.Neg == "Neg" & Cyto.Fusion.Molecular.Risk_update == "Standard" ~ "Low",
              TRUE ~ Cyto.Fusion.Molecular.Risk_update), 
      levels = c("Standard","High", "Low"))) %>% 
    
    mutate(lncRNA_plus_CM_Risk_basedonEFS=factor(case_when(
              lnc.Pos.Neg == "Neg" & Cyto.Fusion.Molecular.Risk_update == "High" ~ "Standard",
              lnc.Pos.Neg == "Pos" & Cyto.Fusion.Molecular.Risk_update == "Standard" ~ "High",
              TRUE ~ Cyto.Fusion.Molecular.Risk_update), 
      levels = c("Standard","High", "Low"))) %>% 
  
    mutate(lncRNA_plus_CM_Risk_2Groups=factor(case_when(
              lnc.Pos.Neg == "Pos" & Cyto.Fusion.Molecular.Risk_update == "Low" ~ "Neg",
              TRUE ~ lnc.Pos.Neg), 
      levels = c("Neg","Pos"))) %>% 
...
#Set factor levels and fix error  
CDE.survdat <- CDE.survdat %>%
  ...
  mutate_at(vars(matches("LSC\\.[Mq]")),~factor(.,levels=c("low","high"))) %>%
  mutate_at(vars(matches("LSC\\.Perc")),~factor(.,levels=c("q1","q2",
                                                           "q3","q4"))) %>%
  mutate_at(vars(matches("lnc.Pos")), ~factor(., levels=c("Neg","Pos"))) %>% 
  mutate_at(vars(matches("lnc.Med")), ~factor(.,levels=c("low","high"))) %>%
  mutate_at(vars(matches("lnc\\.Perc")),~factor(.,levels=c("q1","q2",
                                                           "q3","q4"))) %>%
  #this has not been explicitly updated yet 7/24/20 even though Final.Risk.Group has been 
  mutate_at(vars(lncRNA_plus_CM_Risk),
            ~factor(., levels=c("Standard","Low","High"))) %>%
  
  #Need to re-run this b/c NPM1/CEBPA was swapped
...
head(CDE.survdat[,1:5])
dim(CDE.survdat) #1298  223

# write.csv(CDE.survdat, "TARGET_AML_lncRNA_CDEs_09.14.21.csv", row.names = FALSE)
```

```{r}
# table(CDE.survdat$ETS_Fusion, useNA='ifany')
# table(CDE.survdat$Primary.CNV, useNA = 'ifany')

#No real issues with eligibility! whoo!
CDE.survdat %>% 
  group_by(Set,Eligibility_Comments) %>%
  summarise(N=n()) %>% 
  ungroup() #%>% 
  # write.csv(., "TARGET_AML_lncRNA_Eligability.csv", row.names = F)

# table(CDE.survdat$Eligibility_Comments) # 2 must be removed 
# filter(CDE.survdat, Eligibility_Comments=="remove") %>% 
#   select(USI,Reg., Set, Eligibility, Eligibility_Comments)
# table(CDE.survdat$Major.Group_update, useNA='ifany')
# table(CDE.survdat$lncRNA_plus_CM_Risk_basedonEFS)

CDE.survdat %>% 
  filter(SCT.in.1st.CR != "Unknown") %>% 
  group_by(Cyto.Fusion.Molecular.Risk_update, SCT.in.1st.CR) %>% 
  summarise(N=n())


CDE.survdat %>% 
  filter(SCT.in.1st.CR != "Unknown") %>% 
  group_by(lncRNA_plus_CM_Risk_basedonEFS, SCT.in.1st.CR) %>% 
  summarise(N=n())


CDE.survdat %>% 
  filter(SCT.in.1st.CR != "Unknown", Cyto.Fusion.Molecular.Risk_update != "Low") %>% 
  group_by(lnc.Pos.Neg,Cyto.Fusion.Molecular.Risk_update, SCT.in.1st.CR) %>% 
  summarise(N=n())
...
```{r}
Expn_Data_Supp <- CDE.survdat %>% 
  select(USI,Set,lnc.Pos.Neg,lnc.Score, matches("ENSG")) %>% 
  select(-all_of(LSC17$gene_id))


head(Expn_Data_Supp)
dim(Expn_Data_Supp) 
# write.csv(Expn_Data_Supp,
#           paste0("manuscript/Supplemental_Tables/TARGET_AML_Expn_Data_Supplement",date,".csv"),
#                  row.names = FALSE)
```

```{r warning=FALSE}
CDE_Data_Supp <- CDE.survdat %>% 
  select(-matches("^ENSG")) %>% 
  # left_join(., JEFedited, by="USI") %>%
  arrange(Set) %>%
  mutate(Major.Group=Major.Group_update,
         `Cyto/Fusion/Molecular Risk update`=Cyto.Fusion.Molecular.Risk_update) %>% 
  mutate_all(~as.character(.)) %>% 
  select(USI,
         Study=Protocol,
         Set,
         `lncScore Group`=lnc.Pos.Neg, 
         `LSC17 Group`=LSC.Med.Groups_AllPts,
         `Integrated lncScore CMrisk`=lncRNA_plus_CM_Risk,
         `Major Group (for stratified randomization)`=Major.Group,
         `Primary Fusion`=Primary.Fusion,
         `Primary CNV`=Primary.CNV,
         `Additional CNV or Fusion`=Additional.Fusions.CNV,
         # `Cyto vs. Seq`=Cyto.vs..Seq,
         `Cyto/Fusion/Molecular Risk update`,
         `Final Risk Group Update`=Final.Risk.Group, 
         `EFS time (days)`=EFS.time..days.,
         `OS time (days)`=OS.time..days.,
         `EFS event type ID`=EFS.event.type.ID,
         `OS event ID`=OS.event.ID)
         # `RAM Positive`=RAM.phenotype, 
          # ISCN)

         # one_of(cols_for_supp$x),
         # EFS.time..days.,
         # OS.time..days.,
...
head(CDE_Data_Supp)
dim(CDE_Data_Supp) #1300   56

# xlsx::write.xlsx(CDE_Data_Supp, "manuscript/COG_Requests/TARGET_AML_lncScore_Manuscript_Meshinchi_09.13.21.xlsx")
# write.csv(CDE_Data_Supp,
#           paste0("manuscript/Supplemental_Tables/TARGET_AML_CDE_Supplement",date,".csv"),
#           row.names = F)
```
...
CDE.survdat %>% 
  group_by(Set) %>% 
  mutate(N=n()) %>% 
  group_by(Cyto.Fusion.Molecular.Risk_update, .add=TRUE) %>% 
  summarize(N_RG=n(),
            Percent=round(N_RG/N[1]*100, digits = 1)) %>% 
  ungroup() %>% 
  select(-N_RG) %>%
  spread(Set, Percent) %>% 
  rename_at(vars(1), ~gsub("Cyt.+lar.","Cytomolecular ",.)) %>%
  rename_at(vars(2:4), ~paste(.,"Set")) %>%
  mutate_at(vars(1), ~paste(.,"Risk")) #%>% 
  # write.csv(.,paste0("manuscript/Supplemental_Tables/lncRNA_Risk.Group_SuppTable_", date, ".csv"), row.names = F)
```
...
CDE.survdat %>% 
  ...
  summarize(N_PG=n(),
            Percent=round(N_PG/N[1]*100, digits = 1)) %>% 
  ungroup() %>% 
  select(-N_PG)%>%
  spread(Set, Percent) %>%
  mutate(Protocol=factor(Protocol,
                         levels=c("CCG-2961","AAML03P1",
                                  "AAML0531","AAML1031"))) %>%
  rename_at(vars(2:4), ~paste(.,"Set")) #%>%
  # write.csv(.,paste0("manuscript/Supplemental_Tables/lncRNA_Protocol_SuppTable_", date, ".csv"), row.names = F)


```

```{r}
# table(CDE.survdat$Set, CDE.survdat$Treatment.Arm)
Fusion.Group.Frequencies <- CDE.survdat %>% 
  group_by(Set) %>% 
  mutate(N=n()) %>% 
  group_by(Major.Group_update, add=TRUE) %>% 
  summarize(N_MG=n(),
            Percent=round(N_MG/N[1]*100, digits = 1)) %>% 
  ungroup() %>% 
  select(-N_MG) %>%
  spread(Set, Percent) %>% 
  rename_at(vars(Train:Validation2), ~paste(., "Set")) %>%
  arrange(desc(`Train Set`)) #%>% 
  # write.csv(.,paste0("manuscript/Supplemental_Tables/lncRNA_FusionGroup__SuppTable_", date, ".csv"), row.names = F)


head(Fusion.Group.Frequencies)
dim(Fusion.Group.Frequencies)
```
...
```{r}
CDE.validation2 <- CDE.0531.subset[CDE.0531.subset$Sequencing_Cohort == "PolyA",]
CDE.validation2$Set <- "Validation2"
```
...
```{r}
# dim(DE.all$DE) #6,664
# sum(rownames(all_DE_lncRNAs) %in% rownames(DE.all$DE))
# write.csv(DE.all$DE %>% 
#             rownames_to_column("gene_id") %>% 
#             left_join(., IDmap, by="gene_id") %>%
#             select(gene_id,gene_name,everything()),
#   "Survival/TraingSet_AMLvsNBM_DE_lncRNA_mRNAs.csv" )
# head(all_DE_lncRNAs)
...
df <- CPM.lnc %>% 
  as.data.frame() %>%
  rownames_to_column("gene") %>% 
  mutate(lncRNA=ifelse(gene %in% lncRNAs$gene_id,
                       "yes","no")) %>% 
  gather(sample,log2_CPM,-gene,-lncRNA) %>%
  mutate(Group=ifelse(grepl("BM[0-9]|RO[0-9]", sample), "NBM","AML"))

# head(df)
dens.plot <- ggplot(data = df, aes(log2_CPM, fill=lncRNA)) +
  geom_density() +
  facet_wrap(~Group) +
  labs(title = "Distribution of lncRNAs in Pediatric AML")
  
dens.plot
# ggsave(filename = "TARGET_AML_lncRNAs_Distribution.svg", plot = dens.plot, device = "pdf",height = 5, width = 7)
```

```{r}
#for compatibility keep all gene in RBD in polyA dataset
polyA.lnc <- polyA[keep,] 
TMM.norm2 <- calcNormFactors(polyA.lnc, method = 'TMM')
lib.size2 <- colSums(polyA.lnc)

polyA.CPM.lnc <- cpm(polyA.lnc, 
               lib.size = lib.size2*TMM.norm2,
               log=TRUE, 
               prior.count = 0.5)

dim(polyA.CPM.lnc) #26157   315
```
...
Targets <- DE$DE %>% 
  ...
  filter(gene_id %in% rownames(CPM.lnc))#remove Transposable elements
  

dim(Targets) #1588   34
length(unique(Targets$gene_id)) #1452 (1431??)
head(Targets[,1:5])
```
...
#targets <- c(targets, "ENSG00000248489","ENSG00000281903") #include adult lncRNAs for just a curiosity question
...
CDE.validation2 <- CDE.validation2 %>% 
    left_join(.,polyA.CPM.lnc[targets,] %>%
              t() %>%
              as.data.frame() %>% 
              rownames_to_column("USI"), 
            by="USI") 
...
ures <- lapply(Genes, cox_reg, CDE=CDE.train) %>% 
  ...
  left_join(., select(DEGs.DF,gene_id,gene_name,
...
dim(sig.ures) #446 univariate significantly associated 
head(sig.ures)
...
extract_metrics_df(uv_upreg_0.1$error.metrics,V=TRUE,V2=TRUE)
uv_upreg_0.1$AIC
...
LSC.lnc.Low <- KM.plots(df=filter(final.mod$Model.Comparison.df,lnc.Med.Groups_AllPts == "low") %>% 
                                mutate(LSC17.win.LncLow=quantcut(LSC17.Score,q=2,na.rm=FALSE, 
                                                                 labels=c("low","high"))), 
                         group_vars = NULL,
...
LSC.lnc.Hi <- KM.plots(df=filter(final.mod$Model.Comparison.df,lnc.Med.Groups_AllPts == "high") %>% 
                                mutate(LSC17.win.LncHi=quantcut(LSC17.Score,q=2,na.rm=FALSE, 
                                                                 labels=c("low","high"))), 
                         group_vars = NULL,
