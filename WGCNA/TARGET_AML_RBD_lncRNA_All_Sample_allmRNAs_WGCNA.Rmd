...
# #                   colnames(rbd.tpm))] 
#  
# keep <- rowSums(rbd.tpm >= 1.0) >= 0.01*ncol(rbd.tpm)
...
```{r}
train.test.set <- read.csv("Survival/TARGET_AML_RBD_lncRNAs_UV_upreg_pvalue_LT0.1_CDE_8.02.19.csv")
...
hubs.df <- as.data.frame(hubs) %>%
  ...
  left_join(., lapply(mods.with.lncSig, function(x) paste(unname(gene.map[x]), collapse = "; ")) %>% 
                      as.data.frame() %>%
                      t() %>% 
                      as.data.frame() %>% 
...
```{r message=FALSE}
library(pathfindR)
SIF <- file.path(SCRATCH,"jlsmith3/lncRNA/PathwayCommons11.All.PathfindR.hgnc.sif")
```
...
```{r}
# lapply(names(GSEAGO), function(x)
#   write.csv(GSEAGO[[x]],
#             paste0("WGCNA/WGCNA_",x,"_Module_GO_Enrichment.csv"),
#             row.names = FALSE))
```
